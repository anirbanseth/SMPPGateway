...
namespace SMSGateway.SMSCClient
{
    public class SmppServer
    {
        ...
        private object deliveryLoaderLock = new object();
        ...
        private void Connection_OnSessionStart(SMPPConnection connection, SessionEventArgs e)
        {
            try
            {
                ...
                if (ReferenceEquals(id = new SmppSessionManager().Save(session).Result, null))
                    throw new Exception("Session creation failed");

                connection.Identifier = session;
            }
            catch (Exception ex)
            {
        ...
        private void Connection_OnSessionEnd(SMPPConnection connection, SessionEventArgs e)
        {
            try
            {
                ...
                SmppSession session = new SmppSessionManager()
                    .Search(new SmppSessionSearch { Id = e.Id })
                    .Result
                    .First();

                session.ValidTo = DateTime.Now;
                new SmppSessionManager().Save(session);
                ...
                )
                {
                    do
                    {
                        ...
                        new SmppDeliveryManager().Save(smppDelivery);
        ...
        private void Connection_OnSent(SMPPConnection connection, SmppEventArgs e)
        {
            try
            {
                ...
                new SmppCommandManager().Save(command);
            }
            catch (Exception ex)
            {
        ...
        private int Connection_OnBindTransceiver(SMPPConnection connection, BindEventArgs e)
        {
            try
            {
                ...
                SmppUser smppUser = new SmppUserManager()
                    .Search(smppUserSearch)
                    .Result
                    .FirstOrDefault();
        ...
        private string Connection_OnSubmitSm(SMPPConnection connection, SubmitSmEventArgs e)
        {
            ...
            try
            {
                ...
                if (new SmppTextManager().Add(c).Result == 0)
                    throw new SmppException(StatusCodes.ESME_RSYSERR);

                Task.Run(async () => { await ProcessMessage(connection, e, c, isMultiPartMessage); }).Wait(100);
        ...
        protected async Task ProcessMessage(SMPPConnection connection, SubmitSmEventArgs e, SmppText c, bool isMultiPartMessage)
        {
            try
            {
                SmppSession session = (SmppSession)connection.Identifier;
                ...
                if (isMultiPartMessage)
                {
                    ...
                    await ConsolidateMessage(connection, e, c);
                }
                else
                {
                    ...
                    smsRecord.Source = e.SourceAddress;
                    smsRecord.Destination = e.DestAddress;
                    ...
                    smsRecord.UserId = session.User.Id;
                    smsRecord.CreatedOn = DateTime.Now;
                    smsRecord.DeliveryStatus = null;
                    smsRecord.DeliveredOn = null;
                    smsRecord.UpdatedOn = DateTime.Now;

                    await new SmppSmsRecordManager().Save(smsRecord);

                    c.RecordId = smsRecord.Id;
                    c.Status = "PRC";
                    await new SmppTextManager().Add(c);
                }
            }
            catch (Exception ex)
            {
        ...
        protected async Task ConsolidateMessage(SMPPConnection connection, SubmitSmEventArgs e, SmppText smppText)
        {
            ...
            if (!ReferenceEquals(smppTexts, null) && smppTexts.Count > 0)
            {
                ...
                for (int i = 0; i < smppTexts.Count; i++)
                {
                    ...
                    await new SmppTextManager().Add(text);
                }
            ...
            if (isReceiveCompleted)
            {
                ...
                smsRecord.Source = smppText.SourceAddress;
                smsRecord.Destination = smppText.DestAddress;
                ...
                smsRecord.UserId = ((SmppSession)connection.Identifier).User.Id;
                smsRecord.CreatedOn = DateTime.Now;
                smsRecord.DeliveryStatus = null;
                smsRecord.DeliveredOn = null;
                smsRecord.UpdatedOn = DateTime.Now;
        ...
        private void Connection_OnDeliverSmTimerTick(SMPPConnection connection)
        {
            ...
            try
            {
                ...
                SmppDelivery[] smppDeliveries = null;
        ...
        {
            SmppDelivery smppDelivery = e.Data;
            ...
            try
            {
                ...
                smppDelivery.CommandId = e.Data.CommandId ?? e.Id;
                if (e.SentStatus == 0)
                {
                    smppDelivery.Status = "SENT";
                }
                else if (smppDelivery.RetryIndex >= SmppDeliverySearch.MAX_RETRY)
                {
        ...
        private uint Connection_OnUnbind(SMPPConnection connection)
        {
            try
            {
                ...
                new SmppSessionManager().Save(session);//.Wait();
